...
use yin_yang_proto::packets::DisplayInfo;
...
impl State {
    ...
    fn ensure_shm_buffer(&mut self, qh: &QueueHandle<Self>) -> Result<()> {
        let format = self
            .shm_format
            .context("compositor did not advertise an SHM format supported by yin")?;
...
fn alloc_shm_buffer(
    ...
    let name = CString::new("yin-shm").expect("literal has no NUL bytes");
